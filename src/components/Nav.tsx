import Link from 'next/link'
import Image from 'next/image'
import MagnifierIcon from '~icons/ant-design/search-outlined.tsx'
import HomeIcon from '~icons/ant-design/home-outlined.tsx'
import MoviesIcon from '~icons/pepicons/clapperboard.tsx'
import TVIcon from '~icons/pepicons/television.tsx'
import styles from './Nav.module.scss'

export function Nav() {
  return (
    <nav className={styles.nav}>
      <ul className="nolist">
        <li className={styles.logo}>
          <Image
            src="data:image/svg+xml;charset=utf-8;base64,"
            width={48}
            height={48}
            alt="solid logo"
          />
        </li>
        <li>
          <Link href="/" aria-label="Home">
            <HomeIcon width={24} height={24} />
          </Link>
        </li>
        <li>
          <Link href="/movie" aria-label="Movies">
            <MoviesIcon width={24} height={24} />
          </Link>
        </li>
        <li>
          <Link href="/tv" aria-label="TV Shows">
            <TVIcon width={24} height={24} />
          </Link>
        </li>
        <li>
          <Link href="/search" aria-label="Search">
            <MagnifierIcon width={24} height={24} />
          </Link>
        </li>
      </ul>
    </nav>
  )
}
